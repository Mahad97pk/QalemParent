 ...
 * `statichtml`, skip entirely for mobile, plain `startDevServer` PTY
 * otherwise → wire an exit
 ...
 * and `kill_port` / `clear_project_cache` invokes.
 *
...
/** Record of a dev-server process that died without Qalem stopping it. */
...
/* Lines that match this pattern are the dev server logging Qalem's
...
export function useDevServer(currentProjectPath: string | null) {
    ...
    (projectPath: string, s: ProjectServerState) => {
      ...
      try {
        handle.pty.onExit(({ exitCode }) => {
          ...
          current.handle = null;
          // Only exits Qalem did NOT initiate reach this point:
  ...
  const startServerForProject = useCallback(
    async (projectPath: string, projectName: string, port: number, windowLabel: string) => {
      ...
      // `.qalem/project.json` → `force_static_serve`; when set, treat it as
      ...
      try {
        // Force-static projects don't need an npm install to preview: the
        // site is served straight off disk regardless of its build tooling.
        const depStatus = forceStatic
   ...
   *  interactive CLI prompts (y/n confirms) right in the logs pane instead
   *  of dead-ending on a read-only view. */
