...
export function useProjectLifecycle({
  ...
  const handleSelectProject = async (
    ...
    const storageKey = `qalem-project-loaded-${windowLabel}`;
    sessionStorage.setItem(storageKey, project.path);

    // Set window title to include project name
    void setWindowTitle(`Qalem - ${project.name}`).catch((error) => {
    ...
    // Projects outside ~/Qalem can enter the app via session restore, URL params,
    ...
    // Ensure .qalem/ is gitignored (backwards compat for existing projects)
    void invoke('ensure_gitignore_has_qalem', { projectPath: project.path }).catch((err) =>
  ...
  const handleImportLocalFolder = async () => {
    ...
    try {
      ...
      const friendly = message.includes('already registered')
        ? "This folder is already in Qalem. To work on a different workspace from the same folder, clone the repo again via 'Import from GitHub' (each clone is independent), or duplicate the folder on disk first."
        : message;
  ...
  const handleBackToProjects = () => {
    ...
    const windowLabel = getWindowLabel();
    const storageKey = `qalem-project-loaded-${windowLabel}`;
    const dismissedKey = `qalem-auto-open-dismissed-${windowLabel}`;
    ...
    void setWindowTitle('Qalem').catch(console.error);
