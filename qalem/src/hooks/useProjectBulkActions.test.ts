...
function makeProject(overrides: Partial<DashboardProject> = {}): DashboardProject {
  ...
  return {
    name,
    path: overrides.path ?? `/Users/test/Qalem/${name.toLowerCase()}`,
    thumbnail: null,
    last_opened: null,
    git_branch: 'main',
    uncommitted_count: 0,
...
describe('useProjectBulkActions', () => {
  ...
  it('removes selected projects from Qalem and unpins pinned projects', async () => {
    const alpha = makeProject({ name: 'Alpha' });
    const beta = makeProject({ name: 'Beta' });
  ...
  it('blocks deleting external projects from the bulk action bar', () => {
    ...
    expect(showToast).toHaveBeenCalledWith(
      'External projects can only be removed from Qalem.',
      'error'
    );
  });
});
