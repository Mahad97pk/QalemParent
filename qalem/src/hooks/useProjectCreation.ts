 ...
 * detaches from the template repo, gitignore `.qalem/`, fire-and-forget
 ...
 * `list_projects` (duplicate-name check), `ensure_qalem_dir`,
...
export function useProjectCreation({ onComplete, onCancel }: UseProjectCreationParams) {
  ...
  const handleCreate = async () => {
    ...
    try {
      // Ensure Qalem directory exists
      const qalemDir = await invoke<string>('ensure_qalem_dir');
      ...
      if (selectedTemplate.id === 'blank') {
        ...
        // Ensure .qalem/ is gitignored to prevent phantom changes
        await invoke('ensure_gitignore_has_qalem', { projectPath: projectPath });
  ...
  const handleCreateFromZip = async () => {
    ...
    try {
      ...
      await invoke('ensure_gitignore_has_qalem', { projectPath });
