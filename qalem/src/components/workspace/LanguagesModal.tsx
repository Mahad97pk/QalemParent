 ...
 *   pick languages, Qalem writes the config directly.
...
export function LanguagesModal({ projectPath, onSendToClaude }: LanguagesModalProps) {
  ...
  const handleSaveOnly = async () => {
    ...
    // Qalem never deletes files, so removed languages leave translated
  ...
  const handleAiFallback = () => {
    ...
    setPromptReview({
      description:
        "Qalem couldn't edit this config automatically. This prompt asks your AI agent to make the change instead.",
  ...
  // Removal needs honest messaging: Qalem never deletes files, and
      ...
      {!promptReview && !isLoading && showSetupFlow && (
        <div className="languages-editor">
          <p className="languages-intro">
            Your project uses the Next.js App Router. Qalem adds multilingual support with{' '}
