...
const PROJECT_VIEW_MODE_STORAGE_KEY = 'qalem-project-view-mode';
  ...
  const handleThumbnailFileSelected = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setProjects((prev) =>
        prev.map((p) =>
          p.path === projectPath
            ? { ...p, thumbnail: '.qalem/thumbnail.png', thumbnailData: dataUrl }
            : p
        )
      );
      ...
      <div className="project-list dashboard">
        ...
        {deleteConfirm && (
          <ProjectActionConfirmModal
            ...
            hint="This removes the local project folder and its files. Use Remove from Qalem if you only want to hide it from the app."
            loading={deleting}
