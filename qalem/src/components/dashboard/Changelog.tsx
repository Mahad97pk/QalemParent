...
const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    items: [
      ...
      'Agent-led onboarding — setting up a new machine? Pick your coding agent and it installs everything else for you (Homebrew, Node, Git, GitHub, your hosting CLI) while Qalem independently verifies each step. The classic wizard stays one click away at all times',
  ...
  },
  {
    ...
    items: [
      ...
      "Per-project workspaces — assign any project to a workspace and its terminals, git, pull requests, and AI all use that workspace's logins automatically. Move a project between workspaces right from the dashboard and Qalem relocates its files for you",
      ...
      'Choose your projects folder — point Qalem at any folder you like (such as an existing ~/Dev directory) instead of ~/Qalem, globally or per workspace, and optionally move your existing projects across',
  ...
  },
  {
    ...
    items: [
      'Mobile app previews — open a React Native, Expo, or Flutter project and the preview pane becomes a real, interactive device. Qalem boots an iOS Simulator or Android emulator for you, builds and launches your app onto it (with the build log streaming in), and mirrors the screen live so you can tap, swipe, and type right in the workspace',
      ...
      'Multilingual sites — Cmd+K → “Languages” to add languages to any Next.js or Astro site. Search every language (not a preset list), pick a default, and Qalem edits your i18n config surgically — it never guesses, and unusual configs fall back to a “Fix with AI” hand-off instead of being overwritten',
  ...
  },
  {
    ...
    items: [
      ...
      "Restarting the dev server no longer crashes Qalem — on some setups it was killing the app's own preview process along with the dev server",
  ...
  },
  {
    ...
    items: [
      ...
      'The toolbar Support button now opens the Qalem community Slack directly',
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      'Monorepo support — Qalem detects pnpm/npm workspaces when you import a repo (or first open an existing project) and asks which app to work on; the dev server, preview, and /public tools run inside that workspace while git and PRs stay at the repo root. "Use the whole repo" skips the picker',
  ...
  },
  {
    ...
    items: [
      ...
      'Open in Browser fix — Preview toolbar\'s "Open in Browser" now opens your real dev server URL (e.g. `localhost:3000`) instead of Qalem\'s internal proxy port',
  ...
  },
  {
    ...
    items: [
      ...
      'Skip broken Claude binaries — if a stale `claude` install is on the GUI PATH (e.g. an old `/opt/homebrew/bin/claude` from a legacy installer), Qalem now validates each candidate with `--version` and falls through to the next working install instead of surfacing the raw npm-wrapper error',
    ],
  },
  {
