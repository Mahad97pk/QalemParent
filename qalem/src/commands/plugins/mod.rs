 ...
 * - Registry: {project}/.qalem/plugins/registry.json
 * - Plugin files: {project}/.qalem/plugins/{plugin-id}/ (plugin.json, dist/, icon.svg)
 * - Plugin data: {project}/.qalem/plugins/{plugin-id}/storage.json
 */
...
pub(crate) fn check_min_app_version(
    ...
    if app_ver < min_ver {
        return Err(format!(
            "Plugin '{}' requires Qalem v{} or later (current: v{}). Please update Qalem.",
...
pub struct PluginManifest {
    ...
    /// Minimum Qalem version required
    #[serde(default)]
