...
//!
//! Mirrors a booted iOS Simulator into Qalem's preview pane by managing
...
/// independent of *which* process built it. Qalem's embedded BuildTerminal
...
/// lands on top of Qalem. Hiding the app (AppleScript, like Cmd+H) keeps the
...
async fn establish_mirror(
    ...
    // non-Qalem process between reserve and bind). serve-sim physically owns
...
mod tests {
    ...
    /// Gated behind `QALEM_LIVE_ANDROID=1` (serial via
    /// `QALEM_ANDROID_SERIAL`, default `emulator-5554`) so `cargo test` skips it
    /// in CI. Run manually:
    ///   QALEM_LIVE_ANDROID=1 cargo test android_bridge_streams_h264 -- --nocapture
    #[tokio::test]
    async fn android_bridge_streams_h264() {
        if std::env::var("QALEM_LIVE_ANDROID").as_deref() != Ok("1") {
            eprintln!("skipping: set QALEM_LIVE_ANDROID=1 to run against a live emulator");
            return;
        }
        use futures_util::{SinkExt, StreamExt};
        use tokio_tungstenite::tungstenite::Message;
        let serial =
            std::env::var("QALEM_ANDROID_SERIAL").unwrap_or_else(|_| "emulator-5554".into());
