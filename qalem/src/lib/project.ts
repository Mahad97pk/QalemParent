 ...
 * - Listing and managing projects in ~/Qalem
...
export interface DashboardProject {
  ...
  /** Whether this project is an external (non-~/Qalem) project */
  is_external: boolean;
 ...
 * Scans ~/Qalem for project folders and enriches with git/deployment info.
...
/**
 * List all projects in the ~/Qalem directory.
...
/**
 * Ensure the ~/Qalem directory exists, creating it if necessary.
 * @returns Absolute path to the Qalem directory
 */
export async function ensureQalemDir(): Promise<string> {
  return invoke<string>('ensure_qalem_dir');
}
 ...
 * arbitrary `~/Qalem` paths for `exists`, and this also covers custom
...
/**
 * Ensure .qalem is included in the project's .gitignore file.
 * Creates .gitignore if it doesn't exist.
 * @param projectPath - Absolute path to the project directory
 */
export async function ensureGitignoreHasQalem(projectPath: string): Promise<void> {
  return invoke<void>('ensure_gitignore_has_qalem', { projectPath });
}
...
/**
 * Remove a project from Qalem without deleting its files.
 ...
 * sessions). Only works for ~/Qalem projects, and is rejected by the
...
export async function startDevServer(
  ...
  // so when Qalem itself runs under `pnpm tauri dev`, those leak into
