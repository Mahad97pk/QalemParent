 ...
 * and settings that persist to `.qalem/project.json`.
 *
...
describe('lib/project', () => {
  ...
  describe('ensureQalemDir', () => {
    it('invokes "ensure_qalem_dir" and returns the absolute path', async () => {
      invokeMock.mockResolvedValue('/Users/test/Qalem');

      const result = await ensureQalemDir();

      expect(invokeMock).toHaveBeenCalledWith('ensure_qalem_dir', {});
      expect(result).toBe('/Users/test/Qalem');
    });

    it('propagates errors from invoke', async () => {
      invokeMock.mockRejectedValue(new Error('mkdir failed'));
      await expect(ensureQalemDir()).rejects.toThrow('mkdir failed');
    });
  });
  ...
  describe('ensureGitignoreHasQalem', () => {
    it('invokes "ensure_gitignore_has_qalem" with projectPath', async () => {
      invokeMock.mockResolvedValue(undefined);

      await ensureGitignoreHasQalem('/abs/project');

      expect(invokeMock).toHaveBeenCalledWith('ensure_gitignore_has_qalem', {
    ...
    it('propagates errors from invoke', async () => {
      ...
      await expect(ensureGitignoreHasQalem('/abs/project')).rejects.toThrow('write failed');
    });
  });
