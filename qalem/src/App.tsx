 ...
 *
 * Under `QALEM_MINIMAL=1` we skip onboarding/dashboard and land on a blank
 * `minimal-workspace` placeholder (Phase 1 gate — shell comes later).
 */
function App({ initialProjectPath }: AppProps) {
  const [boot, setBoot] = useState<'checking' | 'minimal' | 'full'>('checking');
  const [minimalProjectPath, setMinimalProjectPath] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    void getMinimalMode()
      .then((mode) => {
        if (cancelled) return;
        if (mode.enabled) {
          setMinimalProjectPath(mode.projectPath);
          setBoot('minimal');
        } else {
          setBoot('full');
        }
      ...
      .catch((err) => {
        logger.warn('[App] get_minimal_mode failed; booting full app', { error: String(err) });
        if (!cancelled) setBoot('full');
      });
    return () => {
      cancelled = true;
    };
  ...
  // Phase 2: the actual barebones shell — DOM tree, preview + dual code
  // editor, CSS panel. No dashboard, no onboarding, no other subsystems
  // mounted at all under this branch.
  if (boot === 'minimal') {
    if (!minimalProjectPath) {
      return (
        <div
          style={{
            height: '100vh',
    ...
    return <MinimalWorkspace projectPath={minimalProjectPath} />;
  }
...
function AppContents({ initialProjectPath }: AppProps) {
  ...
  const quitConfirmModal = showQuitConfirm && (
    ...
    >
      ...
      >
        <p>Are you sure you want to quit Qalem?</p>
