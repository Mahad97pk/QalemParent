/**
 * Access the Qalem plugin context.
 *
...
export function usePluginContext(): PluginContextValue {
  ...
  const React = (window as any).__QALEM_REACT__;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const CtxRef = (window as any).__QALEM_PLUGIN_CONTEXT_REF__;
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const ctx = (window as any).__QALEM_PLUGIN_CONTEXT__ as PluginContextValue | undefined;
  if (!ctx) {
    throw new Error(
      '@qalem/plugin-sdk: Plugin context not available. ' +
        'Ensure this is called within a Qalem plugin component.'
    );
  }
  return ctx;
}
