...
#### Barebones visual editor (minimal mode)

Phase 1 gate — skips onboarding/dashboard and boots a blank `minimal-workspace`
placeholder. The editor shell (tree / preview / CSS panel) is not built yet.

```bash
SHIPSTUDIO_MINIMAL=1 SHIPSTUDIO_MINIMAL_PROJECT=/absolute/path/to/html-css pnpm tauri dev
```

Flag off → full Ship Studio unchanged.
...
| Barebones visual editor gate (Phase 1) | `SHIPSTUDIO_MINIMAL=1` |
