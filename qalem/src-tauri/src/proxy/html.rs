...
<script>(function(){{
var msg={js_escaped};
window.parent.postMessage({{type:'qalem:error',status:{status_code},message:msg}},'*');
document.getElementById('__ss-err-copy').onclick=function(){{
window.parent.postMessage({{type:'qalem:copy-error',message:msg}},'*');
...
window.parent.postMessage({{type:'qalem:send-error-to-claude',message:msg}},'*')
}};
}})()</script>"#,
    )
}
...
/// and sends a postMessage to the parent so Qalem can log the error.
...
mod tests {
    ...
    fn test_error_overlay_preserves_nav_script() {
        ...
        let result_str = String::from_utf8(result).unwrap();
        assert!(result_str.contains("qalem:navigate"));
        assert!(result_str.contains("qalem:error"));
    }
    ...
    fn test_auth_redirect_interstitial_contents() {
        ...
        assert!(page.contains("qalem:copy-error"));
        assert!(page.contains("qalem:send-error-to-claude"));

        // The nav script rides along so the blank-pane watchdog sees the
        // interstitial as a live page (and the app can log the error).
        assert!(page.contains("qalem:alive"));
        assert!(page.contains("qalem:navigate"));
        assert!(page.contains("qalem:error"));
    }
    ...
    fn nav_script_posts_alive_on_parse() {
        ...
        assert!(NAV_SCRIPT.contains("qalem:alive"));
        assert!(NAV_SCRIPT.contains("qalem:navigate"));
    }
}
