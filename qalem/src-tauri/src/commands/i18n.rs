...
/// next-intl's routing config — the file Qalem manages for App Router
...
pub struct I18nStatus {
    ...
    /// Whether Qalem can manage i18n for this project.
    pub supported: bool,
...
fn replace_locales_in(
    ...
    let loc_idx = find_key_value(src, "locales", Some(1))
        ...
        .ok_or("The existing config has no `locales` array Qalem can update.")?;
    ...
    if has_non_string {
        return Err(
            "Some locales use advanced configuration (custom paths or codes) that Qalem \
    ...
    let def_idx = find_key_value(src, "defaultLocale", Some(1))
        .ok_or("The existing config has no `defaultLocale` Qalem can update.")?;
    let def_quote = *src
        .as_bytes()
        .get(def_idx)
        .filter(|q| **q == b'"' || **q == b'\'')
        .ok_or("`defaultLocale` isn't a plain string Qalem can update.")?;
...
fn apply_i18n_to_content(
    ...
    if let Some(val_idx) = find_key_value(content, "i18n", Some(1)) {
        if content.as_bytes().get(val_idx) != Some(&b'{') {
            return Err(
                "An `i18n` key exists in the config but isn't a plain object Qalem can update."
                    .to_string(),
            );
        }
        ...
        let insert_at = find_exported_object_open(content)
            .ok_or("Qalem couldn't find where to add i18n settings in this config file.")?;
    ...
    let Some(routing_rel) = find_next_intl_routing(workspace) else {
        let mut status = unsupported_status(
            ...
            "This Next.js project uses the App Router, which has no built-in i18n. \
             Qalem can set it up with next-intl — the standard library for \
             App Router projects.",
        );
