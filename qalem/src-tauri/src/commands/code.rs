...
mod tests {
    ...
    fn test_save_project_file_roundtrip() {
        ...
        // root (default ~/Qalem), so the fixture must live there.
        let root = crate::utils::projects_root().expect("projects root");
        let dir = root.join(format!("qalem-code-save-test-{}", std::process::id()));
    ...
    fn test_should_skip_path() {
        ...
        assert!(should_skip_path(Path::new(".qalem")));
        assert!(should_skip_path(Path::new("src/.qalem")));
