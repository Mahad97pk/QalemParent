...
//!
//! Commands for registering and managing projects that live outside ~/Qalem.
...
/// ~/Qalem; external projects live anywhere on disk, so we widen the scope
...
pub async fn register_external_project(app: AppHandle) -> Result<Option<String>, CommandError> {
    ...
    // restored even when they were blank, git-only, or Qalem metadata-only.
...
/// Clear the `workspace_subpath` field in a project's `.qalem/project.json`
...
///
/// Called automatically when a project outside ~/Qalem is opened
/// (e.g., via session restore or URL params) to ensure backend commands
/// don't fail with "Security error: path is outside Qalem directory".
///
/// Returns Ok(true) if newly registered, Ok(false) if already registered or inside ~/Qalem.
#[tauri::command]
#[tracing::instrument(skip(app))]
