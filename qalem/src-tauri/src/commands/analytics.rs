//! # Analytics Commands
//!
//! PostHog analytics integration for Qalem.
...
// Telemetry disabled for this fork. Do not reuse Qalem's upstream keys —
// they are the upstream maintainers' paid ingest quota, not this project's.
...
fn send_event(event_name: &str, distinct_id: &str, properties: serde_json::Value) {
    ...
    // Telemetry disabled for this fork (empty key/host) — bail before doing
    // any work, rather than relying on the HTTP request failing.
    if POSTHOG_API_KEY.is_empty() || POSTHOG_HOST.is_empty() {
        return;
    }
    ...
    if !props.contains_key("$screen_name") {
        props.insert(
            ...
            serde_json::Value::String("Qalem".to_string()),
        );
    }
    props.insert(
    ...
    props.insert(
        "$lib".to_string(),
        serde_json::Value::String("Qalem App".to_string()),
    );
