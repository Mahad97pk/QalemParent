...
//! - macOS: `~/Library/Application Support/Qalem/pins.json`
//! - Windows: `%LOCALAPPDATA%/Qalem/pins.json`
//! - Linux: `$XDG_DATA_HOME/qalem/pins.json`
//!
...
pub(crate) fn get_pins_file_path() -> PathBuf {
    ...
    {
        dirs::home_dir()
            .map(|h| h.join("Library/Application Support/Qalem/pins.json"))
            .unwrap_or_else(|| PathBuf::from("/tmp/qalem-pins.json"))
    }

    #[cfg(target_os = "windows")]
    {
        dirs::data_local_dir()
            .map(|d| d.join("Qalem/pins.json"))
            .unwrap_or_else(|| PathBuf::from("C:/temp/qalem-pins.json"))
    }
