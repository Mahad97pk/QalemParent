...
//!
//! Per-project UI preferences stored in `.qalem/project.json`:
...
pub async fn mark_project_opened(project_path: String) -> Result<(), CommandError> {
    ...
    let qalem_dir = project.join(".qalem");
    ...
    if !qalem_dir.exists() {
        std::fs::create_dir_all(&qalem_dir)
...
pub async fn get_branch_prefix_preference(project_path: String) -> Result<bool, CommandError> {
    let project = validate_project_path(&project_path)?;
    let metadata_path = project.join(".qalem").join("project.json");
...
/// `account_id` in `.qalem/project.json`. The project folder is not
...
/// Write a project's Workspace tag into its `.qalem/project.json`. The
...
/// reading `.qalem/project.json`'s `account_id`. Shared by env-injection
...
pub fn project_account_id_sync(project_path: &std::path::Path) -> String {
    let metadata_path = project_path.join(".qalem").join("project.json");
    let metadata = std::fs::read_to_string(&metadata_path)
        .ok()
        .and_then(|contents| serde_json::from_str::<ProjectMetadata>(&contents).ok());
