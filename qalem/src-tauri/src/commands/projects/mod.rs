...
//! - `metadata` — reading/writing `.qalem/project.json` metadata
...
/// Sync helper for ensuring .qalem/ is in gitignore
fn ensure_gitignore_has_qalem_sync(project: &std::path::Path) -> Result<(), String> {
...
/// Accepts any directory inside ~/Qalem that has project files,
/// a .gitignore (blank projects), or a .qalem metadata folder.
pub(crate) fn is_valid_project(path: &std::path::Path) -> bool {
    path.is_dir()
        && (path.join("package.json").exists()
            || detection::has_html_files(path)
            || path.join(".gitignore").exists()
            || path.join(".qalem").exists()
...
fn save_removed_projects_config(config: &RemovedProjectsConfig) -> Result<(), String> {
    ...
    if let Some(parent) = config_path.parent() {
        std::fs::create_dir_all(parent)
            .map_err(|e| format!("Failed to create .qalem directory: {e}"))?;
    }
...
pub async fn list_projects() -> Result<Vec<ProjectInfo>, CommandError> {
    let qalem_dir = crate::utils::projects_root()?;
    ...
    let entries = std::fs::read_dir(&qalem_dir).map_err(|e| e.to_string())?;
    ...
    for entry in entries {
        ...
        if is_valid_project(&path) {
            ...
            let thumbnail_path = path.join(".qalem").join("thumbnail.png");
            ...
            let metadata_path = path.join(".qalem").join("project.json");
    ...
    if let Ok(ext_config) = crate::commands::external_projects::load_config() {
        for ext in &ext_config.projects {
            ...
            if ext_path.exists() && is_valid_project(ext_path) {
                ...
                let thumbnail_path = ext_path.join(".qalem").join("thumbnail.png");
                let thumbnail = if thumbnail_path.exists() {
                    Some(thumbnail_path.to_string_lossy().to_string())
                } else {
                    None
                ...
                let metadata_path = ext_path.join(".qalem").join("project.json");
                let metadata = if metadata_path.exists() {
                    std::fs::read_to_string(&metadata_path)
                        .ok()
                        .and_then(|contents| {
...
pub async fn get_dashboard_projects() -> Result<Vec<DashboardProject>, CommandError> {
    ...
    for entry in entries {
        ...
        if is_valid_project(&path) {
            ...
            // Ensure .qalem/ is gitignored
            let _ = ensure_gitignore_has_qalem_sync(&path);
...
/// Ensures .qalem/ is in the project's .gitignore
#[tauri::command]
#[tracing::instrument(fields(project = %project_path))]
pub async fn ensure_gitignore_has_qalem(project_path: String) -> Result<(), CommandError> {
    let project = validate_project_path(&project_path)?;
    let gitignore_path = project.join(".gitignore");
...
pub async fn create_blank_project(project_path: String) -> Result<(), CommandError> {
    ...
    // Add .qalem/ to gitignore
    let gitignore = path.join(".gitignore");
    std::fs::write(&gitignore, ".qalem/\n")
...
/// Deletes a project directory. Only allows deletion from ~/Qalem.
...
pub async fn delete_project(path: String) -> Result<(), CommandError> {
    ...
    // below can't be defeated by a lexical path like `~/Qalem/../../.ssh`.
    ...
    if crate::commands::external_projects::is_registered_external_path(&canonical)? {
        return Err(
            "Cannot delete external projects. Use 'Remove from Qalem' instead."
                .to_string()
                .into(),
        );
    }
...
/// Removes a project from Qalem's dashboard without deleting its files.
///
/// Projects inside a configured projects folder are discovered automatically, so
/// this records the exact project path in Qalem's app config and list
...
pub async fn remove_project_from_app(path: String) -> Result<(), CommandError> {
    ...
    {
        return Err(
            "Can only remove projects that live in a Qalem projects folder."
                .to_string()
                .into(),
        );
    }
...
///
/// Only ~/Qalem projects can be renamed (external projects are rejected,
...
/// the directory — git remotes, `.vercel`, `.qalem` metadata — travels
...
pub async fn rename_project(
    ...
    // lexical, so checking the raw `old_path` would let `~/Qalem/../../foo`
    ...
    // Reject external projects (their folders live outside ~/Qalem).
...
/// Hidden dirs (e.g. the `.qalem` app-config dir, which stays at the default
...
mod tests {
    ...
    fn make_project(root: &std::path::Path, name: &str) {
        ...
        std::fs::write(dir.join(".gitignore"), ".qalem/\n").unwrap();
    }
    ...
    fn scan_movable_buckets_clean_collision_and_skips_hidden() {
        ...
        std::fs::create_dir_all(from.path().join(".qalem")).unwrap();
