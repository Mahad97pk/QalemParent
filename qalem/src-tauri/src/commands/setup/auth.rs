...
pub async fn install_version(app: tauri::AppHandle, version: String) -> Result<(), CommandError> {
    ...
    let temp_dir = std::env::temp_dir().join("qalem-rewind");
...
async fn install_version_platform(
    ...
    // Find current app bundle path (e.g., /Applications/Qalem.app)
    let exe = std::env::current_exe().map_err(|e| format!("Cannot find app path: {e}"))?;
    let app_bundle = exe
        .parent() // MacOS
        .and_then(|p| p.parent()) // Contents
        .and_then(|p| p.parent()) // Qalem.app
    ...
    let extracted_app = extract_dir.join("Qalem.app");
