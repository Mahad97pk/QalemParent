...
const KEYCHAIN_PREFIX: &str = "qalem-account-";
...
/// paths (`~/.qalem/accounts/<id>/`), keychain service names, or env vars.
///
...
/// Root directory for an account's isolated config: `~/.qalem/accounts/<id>/`
...
/// walk in. We also tighten the `~/.qalem/accounts/<id>` parent so the
...
/// get an isolated directory under `~/.qalem/accounts/<id>/`.
...
/// an isolated directory under `~/.qalem/accounts/<id>/`.
...
const TOKEN_PLACEHOLDER: &[u8] = b"sk-ant-[redacted by Qalem]";
...
pub fn disconnect_claude_account(id: String) -> Result<(), CommandError> {
    ...
    if id == DEFAULT_ACCOUNT_ID {
        return Err(CommandError::Validation {
            field: "id".into(),
            reason: "The Default workspace's Claude login isn't managed by Qalem; \
...
mod tests {
    ...
    fn keychain_service_uses_prefix() {
        assert_eq!(keychain_service("default"), "qalem-account-default");
        assert_eq!(keychain_service("abc-123"), "qalem-account-abc-123");
    }
    ...
    fn redact_captures_token_and_replaces_it_in_the_stream() {
        ...
        assert!(shown.contains("sk-ant-[redacted by Qalem]"));
