...
pub async fn read_env_file(file_path: String) -> Result<Vec<EnvVar>, CommandError> {
    // Constrain reads to files inside Qalem/registered projects so this
...
pub async fn write_env_file(file_path: String, vars: Vec<EnvVar>) -> Result<(), CommandError> {
    // Constrain writes to files inside Qalem/registered projects so this
...
/// Validates both project path (must be in Qalem) and filename.
#[tauri::command]
...
pub async fn create_env_file(
    ...
    // Validate project path is inside Qalem directory
    let project = validate_project_path(&project_path)?;
...
pub async fn delete_env_file(file_path: String) -> Result<(), CommandError> {
    // Validate the file is inside Qalem (or a registered external project)
