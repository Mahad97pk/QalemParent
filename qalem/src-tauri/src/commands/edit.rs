...
<section class="hero hero--{{ section.settings.style }}">
  ...
  {% endif %}
    ...
    fn write_back_replaces_only_the_value() {
        ...
        // adds path validation we can't exercise outside the Qalem root).
    ...
    fn tailwind_detection_resolves_monorepo_workspace_subpath() {
        ...
        // .qalem/project.json points the workspace at the subfolder.
        let ss = dir.join(".qalem");
    ...
    /// `validate_project_path`, which only admits ~/Qalem paths).
    fn insert_class(
        root: &Path,
        sig: &ElementSignature,
        class: &str,
