...
pub fn init_sentry() {
    ...
    // Telemetry disabled for this fork (no DSN configured) — bail before
    // calling sentry::init, rather than relying on it to no-op on empty DSN.
...
fn get_log_dir() -> PathBuf {
    ...
    {
        dirs::home_dir()
            .map(|h| h.join("Library/Logs/Qalem"))
            .unwrap_or_else(|| PathBuf::from("/tmp/qalem-logs"))
    }

    #[cfg(target_os = "windows")]
    {
        dirs::data_local_dir()
            .map(|d| d.join("Qalem/logs"))
            .unwrap_or_else(|| PathBuf::from("C:/temp/qalem-logs"))
    }
...
pub fn init_logging() -> Result<(), String> {
    ...
    let file_appender = tracing_appender::rolling::daily(&log_dir, "qalem.log");
