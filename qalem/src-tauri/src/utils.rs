//! # Shared Utilities
//!
//! This module contains shared utility functions used across the Qalem backend.
...
fn get_login_shell_path() -> Option<String> {
    ...
    let mut child = create_command(&shell)
        .args(["-lic", "echo \"__QALEM_PATH__${PATH}\""])
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .spawn()
    ...
    let result = match rx.recv_timeout(Duration::from_secs(3)) {
        Ok(buf) => buf
            .lines()
            .rev()
            .find_map(|line| line.trim().strip_prefix("__QALEM_PATH__"))
...
/// The built-in default projects root, `~/QalemProjects`.
///
/// This always remains a valid location even when the user configures a custom
/// root, so projects already living in `~/QalemProjects` keep opening.
pub fn default_projects_root() -> Result<std::path::PathBuf, String> {
    let home = dirs::home_dir().ok_or("Could not find home directory")?;
    Ok(home.join("QalemProjects"))
}

/// The directory Qalem uses to list and create projects.
///
/// Resolves the user-configured root from persisted app state (cached), falling
/// back to `~/Qalem`. A configured path that no longer exists on disk falls
...
/// setting that predated per-workspace folders); otherwise `~/Qalem`.
fn account_root_in(
...
pub(crate) fn allowed_project_roots() -> Vec<std::path::PathBuf> {
    ...
    // Legacy default root from before the Ship Studio → Qalem rebrand. Kept
    // permanently (not just as a one-time migration step) so projects created
    // under the old `~/ShipStudio` default before this fork's rename keep
    // opening without requiring the user to move anything on disk.
    if let Some(home) = dirs::home_dir() {
...
/// configured root or the default `~/Qalem`) or is a registered external
...
/// Validates a path to a *file* that lives inside ~/Qalem (or a registered
...
/// cache invalidates as soon as anything writes to .qalem/project.json
...
pub fn resolve_workspace_path(project_root: &std::path::Path) -> std::path::PathBuf {
    ...
    let metadata_path = project_root.join(".qalem").join("project.json");
...
/// active workspace subfolder (`workspace_subpath` in `.qalem/project.json`).
///
...
mod tests {
    ...
    /// checking paths inside/outside `~/Qalem`. The tests do not create
    ...
    mod validate_project_path_tests {
        ...
        fn qalem_root() -> std::path::PathBuf {
            dirs::home_dir().expect("home dir").join("QalemProjects")
        }

        #[test]
        fn rejects_relative_path_to_cwd_when_outside_qalem() {
            // Current working directory in test runner is src-tauri, which is
            // outside ~/Qalem. `.` canonicalizes to cwd, so this should
            // fail the security check.
            let err = validate_project_path(".").expect_err("should reject");
            assert!(
                err.contains("Security error") || err.contains("outside Qalem"),
        ...
        fn rejects_nonexistent_path() {
            let err = validate_project_path("/this/path/definitely/does/not/exist/qalem-test")
        ...
        fn rejects_path_traversal_attempt() {
            // ../../etc canonicalizes outside Qalem (if it canonicalizes
            // at all on the test machine), so the validation must reject it.
            let result = validate_project_path("../../../../../../etc");
            assert!(
                result.is_err(),
                "path traversal outside Qalem must be rejected"
            );
        }
        ...
        fn rejects_arbitrary_root_path() {
            ...
            assert!(result.is_err(), "/tmp is outside Qalem, must reject");
        }

        #[test]
        fn accepts_path_inside_qalem_root() {
            // Create a temp project directory inside ~/Qalem just for this test.
            let root = qalem_root();
            if fs::create_dir_all(&root).is_err() {
                eprintln!("skipping: couldn't create ~/Qalem");
                return;
            }
            ...
            assert!(
                result.is_ok(),
                "path inside ~/Qalem should validate, got {result:?}"
            );
        }
        ...
        /// A symlink inside `~/Qalem` that points OUTSIDE it must be
        /// rejected after canonicalization. This covers the classic
        /// path-traversal-via-symlink escape.
        #[test]
        #[cfg(unix)]
        fn rejects_symlink_escape_outside_qalem_root() {
            ...
            let _ = fs::remove_file(&link_path); // clean up from prior failure
                                                 ...
                                                 // be outside ~/Qalem on any Unix-like test machine.
            ...
            assert!(
                result.is_err(),
                "symlink pointing outside Qalem must be rejected after canonicalization, got {result:?}"
            );
        }

        /// External registered project paths (added via the Import flow) must
        /// be accepted even though they live outside `~/Qalem`. We exercise
    ...
    mod validate_project_file_path_tests {
        ...
        #[test]
        fn rejects_file_outside_qalem() {
            // ~/.zshenv is the canonical RCE target — must be rejected.
            let home = dirs::home_dir().expect("home dir");
            let target = home.join(".zshenv-qalem-audit-test");
            let err = validate_project_file_path(&target.to_string_lossy())
                .expect_err("file in $HOME (outside Qalem) must be rejected");
            assert!(
                err.contains("Security error") || err.contains("outside Qalem"),
                "unexpected error: {err}"
            );
        ...
        #[test]
        fn rejects_traversal_out_of_qalem() {
            // A path whose parent canonicalizes outside the root must fail even
            // though the leading segment names Qalem.
            let root = qalem_root();
        ...
        #[test]
        fn accepts_nonexistent_file_inside_qalem() {
            ...
            assert!(
                result.is_ok(),
                "not-yet-created file inside Qalem should validate, got {result:?}"
            );
        }
        ...
        fn rejects_symlinked_final_component() {
            ...
            // Point at /tmp/... outside Qalem; target need not exist.
